...
for {set i 0} {$i < $N} {incr i} {
    ...
    $ns duplex-link $left_gw $left_n($i) $inputlineRate [expr $RTT/6] RED
    ...
    $ns duplex-link $right_gw $right_n($i) $inputlineRate [expr $RTT/6] RED
	$ns queue-limit $right_n($i) $right_gw $B
	$ns queue-limit $right_gw $right_n($i) $B
}

$ns simplex-link $left_gw $right_gw $lineRate [expr $RTT/6] RED
$ns simplex-link $right_gw $left_gw $lineRate [expr $RTT/6] RED
...
for {set i 0} {$i < $N} {incr i} {
	...
	$ns at 0.0 "$tcp($i) advance-bytes 10000000000"
}
