...
Agent/DCTCP set ecn_ 1
Agent/DCTCP set old_ecn_ 1
Agent/DCTCP set dctcp_g_ 0.0625
Agent/DCTCP set ecn_syn_ true
Agent/DCTCP set window_ 180
Agent/DCTCP set slow_start_restart_ false
Agent/DCTCP set packetSize 1454
Agent/DCTCP set tcpTick_ 0.000001
Agent/DCTCP set minrto_ 0.0002
Agent/DCTCP set windowOption_ 0
Agent/DCTCP set ssthresh_ 1
Agent/DCTCP set max_ssthresh_ 1
Agent/DCTCP set tcpip_base_hdr_size_ 84

Agent/DCTCP set nodelay_ true
Agent/DCTCP set segsize_ 1454
Agent/DCTCP set segsperack_ 1
Agent/DCTCP set interval_ 0
Agent/DCTCP set state_ 0
...
for {set i 0} {$i < $numAggr} {incr i} {
    ...
    for {set j $coreIndex} {$j < $coreIndex+4} {incr j} {
        $ns duplex-link $dcAggr($i) $dcCore($j) $linkBW $linkDelay DropTail
...
for {set i 0} {$i < $numTor} {incr i} {
    ...
    for {set j $aggrIndex} {$j < $aggrIndex+4} {incr j} {
        $ns duplex-link $dcTor($i) $dcAggr($j) $linkBW $linkDelay DropTail
...
$ns simplex-link $dcTor(0) $dcNode(0) $linkBW $linkDelay DropTail
...
for {set i 1} {$i < $numNode} {incr i} {
    ...
    $ns simplex-link $dcTor($torIndex) $dcNode($i) $linkBW $linkDelay DropTail
...
for {set i 0} {$i < $N} {incr i} {
    set receiver($i) [new Agent/DCTCP]
...
puts "All links and nodes Ready."
