...
set creditRate [expr 64734895*1] ;# bytes / sec
...
for {set i 0} {$i < $N} {incr i} {
  if {$i % 2} {
    ...
    [[$ns link $left_gateway $left_node($i)] queue] set token_refresh_rate_ [expr $creditRate*4]
    ...
    [[$ns link $right_gateway $right_node($i)] queue] set token_refresh_rate_ [expr $creditRate*4]
  }
}

$ns duplex-link $left_gateway $right_gateway $linkBW $linkLatency XPassDropTail
[[$ns link $left_gateway $right_gateway] queue] set token_refresh_rate_ [expr $creditRate]
[[$ns link $right_gateway $left_gateway] queue] set token_refresh_rate_ [expr $creditRate]
...
for {set i 0} {$i < $N} {incr i} {
  if {$i % 2} {
    ...
    $ns at $nextTime2 "$sender($i) advance-bytes 500000000"
  }
